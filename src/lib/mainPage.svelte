<script lang="ts">
	...
	import { fetchData, type Manga } from './FetchData';
	// prettier-ignore
	...
	async function searchMangaData() {
		...
		isLoading = true;
		mangaList = await fetchData(`https://api.consumet.org/meta/anilist-manga/${searchQuery}`);
		isLoading = false;
	}

	function modalTrigger(title: string, img: string, desc: string, modalId: string) {
		id = modalId;
		...
		modalComponentRegistry['modalComponentOne'].props = { mangaId: modalId };
...
<div class="container px-12 h-full mx-auto mt-10 flex justify-center items-center">
		...
		<div class="space-y-5">
			...
			<form>
				<div class="input-group input-group-divider grid-cols-[auto_1fr_auto] h-10">
					...
					<input class="px-4" bind:value={query} type="search" placeholder="Search..." />
		...
		<div class="flex flex-wrap gap-12 justify-center">
			{#if isLoading == false}
				{#each mangaList as manga}
					<div
						on:click={() =>
							modalTrigger(manga.title.romaji, manga.image, manga.description, manga.id)}
